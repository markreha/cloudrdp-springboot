...
//		CreateSwarmServiceInfo info = new CreateSwarmServiceInfo(image.getInstance().toLowerCase(), name, container.getCpu(), container.getStorage(), replicas, 80, 1000);
//		String id = api.createSwarmService(info);
		//this will report a dead code warning, this is just to get the application working right now
		String id = null;
		if (id != null) {
			// Log success
